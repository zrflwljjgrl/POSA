...
if __name__ == "__main__":
    ...
    darknet_model_v3=load_model(model_path, weights_path)
    darknet_model_v3.eval()
    yolov3 = load_visible_model()
    #加载yolov2的模型
    ...
    # 创建保存干净样本检测结果的目录
    output_dir = "../new_labels/clean_sample"
    ...
    for img_path in pos_image_paths:
        ...
        conf_thres = 0.6
        with torch.no_grad():
            visible_det = Variable(visible_det.type(Tensor))
            detections = darknet_model_v3(visible_det)
            detections = non_max_suppression(detections, conf_thres, nms_thres)
        tensor_data = detections[0]  # 假设detections是一个包含单个Tensor的列表
        bbox_v3 = tensor_data[:, :4].tolist()
        confidences_v3 = tensor_data[:, 4].tolist()
        class_ids_v3 = tensor_data[:, 5].tolist()
        ...
        with open(output_path, 'w') as f_out:
            ...
            for i in range(len(bbox_v3)):
                # 解析检测结果的7个值
                x1 = bbox_v3[i][0]  # 检测框左上角x（在416x416输入下的坐标）
                y1 = bbox_v3[i][1]  # 检测框左上角y
                x2 = bbox_v3[i][2]  # 检测框右下角x
                y2 = bbox_v3[i][3]  # 检测框右下角y
                conf = confidences_v3[i]  # 置信度
                cls_id = int(class_ids_v3[i])  # 类别索引（如0对应'person'）
                if(cls_id!=0):
                    continue
                # 获取类别名称
        ...
        # detections = yolo_eval(out, H, W, conf_threshold=0.5, nms_threshold=0.5)
        # # 保存检测结果
        # # 保存检测结果
        # img_name = os.path.splitext(os.path.basename(img_path))[0]  # 获取图片名称（不带扩展名）
        # output_path = os.path.join(output_dir, f"{img_name}.txt")
