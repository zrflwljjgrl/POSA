...
def apply_mask_to_image(base_image, output_image, square_mask,file_name):
    """
    将 mask 叠加到原图上。
    ...
    #保存图片的路径
    save_path = "workspace/cross_modal_patch_attack/yolov3_image/empatch-y2/square"
...
def generate_square_patch(image_path,H,W,px,py,s,file_name):
    #加载隐身衣的代码
    ...
    # 调用函数将 mask 叠加到原图上
    combined_image = apply_mask_to_image(base_image,output_image,square_mask,file_name) #base_image原始图片，output_image就是在指定位置贴上补丁，其余位置的像素为0
