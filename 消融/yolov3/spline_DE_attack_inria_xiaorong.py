...
from DE_inria_xiaorong import DifferentialEvolutionAlgorithm
from adv_yolo.load_data import *
from yolo_eval import yolo_eval
from util.visualize import draw_detection_boxes
from generate_squre_xiaorong import generate_square_patch, find_max_iou
import torch
...
def ori_squre(img_path, H, W, px, py, r1, box):
    ...
    square_patch_out = yolov3(square_patch_det)  # 图像经过模型预测
    conf_thres=0.001
    nms_thres=0.4
...
if __name__ == "__main__":
    ...
    final_dir = '../../xiaorong_labels/yolov3/yinshenyi/final'  #不同的补丁保存不同的路径（改）
    ...
    for img_path in pos_image_paths:
        ...
        # 这是后面循环给多个人添加补丁的路径
        final_path = os.path.join(final_dir, file_name)#消融实验仅仅优化一个形状，优化方案是根据bbox中目标最大置信度来优化，目的是降低其中最大的置信度
        points_index_list = []
        eq_points_list = []
        all_circle=[]
        all_area=[]
        visible_img_list = []  #一次性保存多个目标人物的信息，才能在多目标上贴上补丁
        rg=0
        r=0
        r1=0
        r2=0
        ori_squre_confidence_max=-999
        for i in range(L):
            ...
            eq_points_list.append(eq_points)
            # 计算初始方形补丁的大小,并且将补丁贴到指定位置,得到最初的方形补丁的置信度
            # 返回初始方形补丁的置信度的值，与后面补丁变形的方法进行比较
            r1 = int(r1)
            r2 = r / 2  # 限制内圆的范围
            # 这里只需要一个，找方形里面置信度最大的去优化即可
            ori_squre_confidence = ori_squre(img_path, H, W, px, py, r1, box)  # 改代码
            if ori_squre_confidence>ori_squre_confidence_max:
                ori_squre_confidence_max=ori_squre_confidence
            ...
            # Append remaining values to lists
            # 将当前循环的四个坐标值组成一个列表
            current_area = [y_head, y_leg, x_left, x_right]
            # 将当前坐标组添加到总列表中
            all_area.append(current_area)
            current_circle=[px,py]
            all_circle.append(current_circle)
        dea = DifferentialEvolutionAlgorithm(15, 24, points_index_list, eq_points_list, all_circle,
                                                     all_area,
                                                     visible_ori, yolov3,
                                                     confidences, img_path, 20, [1, 0.6], H, W, r, boxes,
                                                     ori_squre_confidence_max, rg, r2, r1)  # 传入的模型要改
