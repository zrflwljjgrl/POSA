...
if __name__ == "__main__":
    ...
    # 数据集路径
    dataset_dir = '../../xiaorong_labels'  # 替换为你的数据集路径
    train_dir = os.path.join(dataset_dir, 'yolov2', 'object_score')#替换这两个地方
    # 获取文件夹中的所有图片路径
    image_folder = os.path.join(train_dir, 'final')  # 假设图片存放在 'images' 文件夹中
    ...
    # 创建保存对抗样本检测结果的目录
    output_dir = "../../xiaorong_labels/yolov2/object_score/detection-results_06/adv_sample"  #替换这两个地方
    # 创建目录（如果不存在）
    os.makedirs(output_dir, exist_ok=True)
    # 循环读取每一张图片,并将预测结果保存为txt文档符合计算AP的要求-
    for img_path in image_paths:
        visible_sample = Image.open(img_path).convert('RGB')
        # 显示图片1
        # visible_sample.show()
