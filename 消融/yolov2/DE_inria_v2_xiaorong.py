...
tmp_dir_vis = '../../xiaorong_labels/yolov2/object_score/tmp_dir_visible'
mask_dir = '../../xiaorong_labels/yolov2/object_score/mask'
final_dir = '../../xiaorong_labels/yolov2/object_score/final'
# 创建目录（如果不存在）
...
def GrieFunc(vardim, x, visible_ori, threat_visible_model, prob_ori_visible, img_name, step_number, h, w, circle, s,
    ...
    for j in range(0, point_length):
        if j == 0:
            ...
            path = tmp_dir_vis + '/{}.png'.format(step_number)
            visible_img = Image.open(path).convert('RGB')
            # visible_sample.show()
            visible_img = trans(visible_img)  # to tensor
            # 增加维度，其实每次送入的只有一个图片
            visible_img = torch.stack([visible_img])  # N C H W
        state = []
        p1 = []
        length = len(x[0])
        ...
        ret, thresh = cv2.threshold(gray, 1, 255,
                                    cv2.THRESH_BINARY_INV)  # 二值化图像，其中thresh是阈值，意味着小于该阈值的像素值会被设置为maxval 255
        ...
        if Patch.max() > 1:
            # 归一化操作：除以 255 将值映射到 [0, 1] 范围
            Patch = Patch / 255.0
        x_adv = visible_img * (1 - mask) + mask * Patch  # 哈德曼乘积用*，矩阵乘法用*，补丁利用掩码加到图像上面去
    ...
    with open(tmp_dir_vis + '/{}.png'.format(step_number), 'rb') as fig:
        ...
        out = threat_visible_model(visible_det)
        ...
        reslut = yolo_eval(out, h, w, conf_threshold=0.001, nms_threshold=0.2)
        # 为了可视化图片
        ...
        max_value = torch.max(confidences)
        # print(f"最大 IoU 值: {max_iou}")
        # print(f"最大 IoU 对应的索引: {max_index}")
    if (max_value.item() < ori_squre_confidence.item()):
        return max_value
    else:
...
class DEIndividual:
    ...
    def generate(self, h, w, rg):
        ...
        self.chrom = np.zeros((points_len, length))
        for i in range(0, length):
            k = np.random.randint(-rg, rg)  # 用k记录每次随机变化的量，保证所有图形都移动同样的距离，这样图形形状就相同
            for j in range(0, points_len):
                self.chrom[j][i] = self.points[j][i] + k
                if i % 2 == 0 and self.chrom[j][i] >= h:
                    self.chrom[j][i] = h - 1
                if i % 2 != 0 and self.chrom[j][i] >= w:
                    self.chrom[j][i] = w - 1
...
class DifferentialEvolutionAlgorithm:
    ...
    def solve(self):
        ...
        for j in range(0, len_x):
            state = []
            p1 = []
            length = int(len(x[0]))  # 前一半是第一个补丁，后一半是第二个补丁
            for i in range(length):
                if i % 2 == 0:
                    p1.append([x[j][i], x[j][i + 1]])
            state.append(p1)
        ...
        while (self.t < self.MAXGEN - 1):
            ...
            if self.t == self.MAXGEN - 1 or k == 5 or self.best.fitness < 0.6:
                ...
                best_length = len(x)
                for j in range(0, best_length):
                    if j == 0:
                        img_final = self.visible_ori
                    else:
                        path = final_dir + '/{}'.format(final_name)
                        img_final = Image.open(path).convert('RGB')
                        # visible_sample.show()
                        img_final = trans(img_final)  # to tensor
                        # 增加维度，其实每次送入的只有一个图片
                        img_final = torch.stack([img_final])  # N C H W
                    state = []
                    p1 = []
                    length = int(len(x[0]))
                    # 两个区域的坐标分别加入到p1和p2中去，这里就只需要一个p1，把坐标添加进去即可，把p2删除了
                    ...
                    if Patch.max() > 1:
                        # 归一化操作：除以 255 将值映射到 [0, 1] 范围
                        Patch = Patch / 255.0
                    x_adv = img_final * (1 - mask) + mask * Patch  # 哈德曼乘积用*，矩阵乘法用*，补丁利用掩码加到图像上面去
                    adv_final = x_adv[0].cpu().detach().numpy()  # 只是去掉批次维度
                    adv_final = (adv_final * 255).astype(np.uint8)
                    adv_x_255 = np.transpose(adv_final, (1, 2, 0))
    ...
    def crossoverOperation(self, i, vi, r2):  # 实现交叉操作
        ...
        len_vi = len(vi)
        ui = np.zeros((len_vi, self.vardim))
        for u in range(0, len_vi):
            px_1, py_1 = self.circle[u][0], self.circle[u][1]  # 掩码的圆心坐标
            y_low, y_high, x_left, x_right = self.region[u][0], self.region[u][1], self.region[u][2], self.region[u][
                3]  # 补丁变形范围限制
            k = np.random.randint(0, self.vardim - 1)
            for j in range(0, int(self.vardim)):  # 第一个补丁
                if j % 2 == 0:
                    dis = compute_dis([vi[u][j], vi[u][j + 1]], px_1, py_1)  # 点离圆心的距离
                    pick = random.random()
                    ...
                    if (pick < self.params[0] or j == k) and (
                            ifcross(self.eq_points[u][j // 2], self.eq_points[u][j // 2 + 1], [vi[u][j], vi[u][j + 1]],
                                    px_1,
                                    py_1) == False \
                            and dis > r2 and vi[u][j] > y_low and vi[u][j] < y_high and vi[u][j + 1] > x_left and vi[u][
                                j + 1] < x_right):
                        # dis>8是不在内圆，后面是为了限制在区域内
                        ui[u][j] = vi[u][j]
                        ui[u][j + 1] = vi[u][j + 1]
                    else:  # 如果不满足条件就保留原来的个体
                        ui[u][j] = self.population[i].chrom[u][j]
                        ui[u][j + 1] = self.population[i].chrom[u][j + 1]
        return ui
