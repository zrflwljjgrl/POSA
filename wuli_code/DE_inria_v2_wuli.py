...
tmp_dir_vis = '../workspace/cross_modal_patch_attack/wuli/object_score/tmp_dir_visible'
mask_dir = '../workspace/cross_modal_patch_attack/wuli/object_score/mask'
final_dir = '../workspace/cross_modal_patch_attack/wuli/object_score/final'
# 创建目录（如果不存在）
...
def GrieFunc(vardim, x, visible_ori, threat_visible_model, prob_ori_visible, img_name, step_number, h, w, circle, s,
    ...
    # 加载yolo补丁代码
    # image = cv2.imread('empatch-y3.png')
    ...
    #     Patch = Patch / 255.0
    Patch = patch_final(mask, image_data) #补丁形状的最小外接矩阵
    # tuxing = mask * Patch
    # tuxing = tuxing.transpose(1, 2, 0)
    # # 转换为 uint8
    # tuxing = tuxing.astype(np.uint8)
    # # 保存为图片
    # Image.fromarray(tuxing).save("output——ljj.png")
    # plt.imshow(tuxing)
    # plt.show(block=True)
    if Patch.max() > 1:
        # 归一化操作：除以 255 将值映射到 [0, 1] 范围
...
class DifferentialEvolutionAlgorithm:
    ...
    def __init__(self, sizepop, vardim, points, eq_points, circle, region, visible_ori, darknet_model, \
                 prob_ori_visible, img_name, MAXGEN, params, h, w, s, box, ori_squre_confidence, rg, r2,r1,Q):
        '''
    ...
    def solve(self):
        ...
        while (self.t < self.MAXGEN - 1):
            ...
            if self.t == self.MAXGEN - 1 or k == 5:
                ...
                tuxing = tuxing.astype(np.uint8)
                # 保存为图片
                Image.fromarray(tuxing).save(f"print/yinshenyi/yinshenyi_{self.Q}.png")
                if Patch.max() > 1:
                    # 归一化操作：除以 255 将值映射到 [0, 1] 范围
                    Patch = Patch / 255.0
