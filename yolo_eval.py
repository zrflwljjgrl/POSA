...
def yolo_filter_boxes(boxes_pred, conf_pred, classes_pred, confidence_threshold=0.6,person_class_id=0):
    """
    ...
    pos_inds = (cls_conf > confidence_threshold).view(-1)& (cls_max_id.view(-1) == person_class_id)#布尔变量输出true或者flase表明cls_conf最后一维度的值是否大于置信度阈值
...
def yolo_eval(yolo_output,H,W, conf_threshold=0.6, nms_threshold=0.4):
    ...
    boxes, conf, cls_max_conf, cls_max_id = yolo_filter_boxes(boxes, conf, classes, conf_threshold,person_class_id=0)
