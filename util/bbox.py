...
def box_transform_inv(box, deltas):
    ...
    """
    #意思是yolov2预测出来的值是偏移量，需要跟先验框进行进一步操作才能得到预测框
    c_x = box[:, 0] + deltas[:, 0] #中心点x,y坐标和w，h框坐标
    c_y = box[:, 1] + deltas[:, 1]
    w = box[:, 2] * deltas[:, 2]
    h = box[:, 3] * deltas[:, 3]
...
def generate_all_anchors(anchors, H, W):
    ...
    A = anchors.size(0)#先验框的数量

    # number of cells
    K = H * W #yolov2模型划分的总的框图
    ...
    shift_x = shift_x.t().contiguous() #矩阵的转置操作
    shift_y = shift_y.t().contiguous()

    # 把上面的内容转为float类型的
    c_x = shift_x.float()
    c_y = shift_y.float()
    #将c_x，c_y调整成一个列向量,centers就形成了网格
    centers = torch.cat([c_x.view(-1, 1), c_y.view(-1, 1)], dim=-1)  # tensor of shape (h * w, 2), (cx, cy)

    # add anchors width and height to centers,合成以后all_anchors里面的四个元素就是(网格的坐标，先验框的大小)每个网格5个先验框
    all_anchors = torch.cat([centers.view(K, 1, 2).expand(K, A, 2),
                             anchors.view(1, A, 2).expand(K, A, 2)], dim=-1)
    #view可以改变张量的形状，-1表示这个维度由pytorch自动推算，4表示第二个维度上有四个元素，根据这个来推断第一个维度的值
